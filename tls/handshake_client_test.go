			...
			go func() {
				...
				if test.checkRenegotiationError != nil {
					...
					if err != nil && newErr == nil {
						t.Logf("checkRenegotiationError failed: %s", err)
						return
					}
					err = newErr
				}
...
func TestHandshakeClientEd25519(t *testing.T) {
	...
	config := testConfig.Clone()
	cert, err := X509KeyPair([]byte(clientEd25519CertificatePEM), []byte(clientEd25519KeyPEM))
	if err != nil {
...
func testVerifyPeerCertificate(t *testing.T, version uint16) {
	...
	verifyPeerCertificateCallback := func(called *bool, rawCerts [][]byte, validatedChains []x509.CertificateChain) error {
	...
	tests := []struct {
	...
	}{
		{
			configureServer: func(config *Config, called *bool) {
				...
				config.VerifyPeerCertificate = func(rawCerts [][]byte, validatedChains []x509.CertificateChain) error {
...
func TestForceSuitesCipherSuiteInclusion(t *testing.T) {
	...
	t.Run("TLSv12GreaseRenegNoForceSuite", func(t *testing.T) {
		testNoForceSuitesIgnores(t,
			VersionTLS12,
			[]uint16{
				TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256,
...
func testNoForceSuitesIgnores(t *testing.T, tlsVersion uint16, givenSuites, expectedSuites []uint16) {
	c, s := localPipe(t)
	defer s.Close()
